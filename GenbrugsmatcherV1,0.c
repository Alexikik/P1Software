...
void change_filters(filter_typ *filters_arr, struct user_typ user);
...
int main(int argc, char* argv[]) {
	...
	rc = sqlite3_open("Mobiltelefoner.db", &db);    	/* Opens the database */
	if(rc) {
		fprintf(stderr, "Kan IKKE aabne databasen: %s\n", sqlite3_errmsg(db));
		exit(0);
	}else
		fprintf(stderr, "Databasen blev aabnet succesfuldt\n");

	user = login_system(db);		// Logs in the user and retrieves user information saved in a user struct
	printf(" Din gruppe og postnummer er: %d og %d\n", user.group, user.postnum);	// Testing
	genbrugsvare(user, db);			// Genbrugssammenligner

	sqlite3_close(db);						// Closes the database
}

/* Functions for genbrugssammenligner */
void genbrugsvare(struct user_typ user, sqlite3 *db) {
	sqlite3_stmt *stmt;             
	sql sql_group[MAX_GROUP];       			/* Initializes struct array to save search results */
	int len = 11, inputId = 0, inputRes;               // len+1 is the amount of search results being shown and calculated
	char sql_ori[MAX_LONG_CHAR], filter_str[MAX_LONG_CHAR];
	filter_typ filters_arr;						// Initializes an array of filter_typ which is an array
	filter_typ *filters_ptr = &filters_arr;		// Makes pointer to array
	
	initial_filters_by_group(filters_ptr, user);			// Initializes the filters set by the users group
	// change_filters(filters_ptr, user);					// Changes the filters
	filter_to_sql(filters_ptr, filter_str, user);			// Makes sql command with filters
	sprintf(sql_ori, "%s ORDER BY Pris", filter_str);		// Finalizes the SQL command
	initialize_data(db, stmt, sql_group, sql_ori, len);     // Saves search results in struct array
	 
	/* Loops until the user wants to leave */
	do{ 
		print_topX(sql_group, len);             // Prints top "x" (x is defined by "len")
		printf("\nIndtast nummeret ud fra det produkt du oensker at se: \n (Tast [0] for at aendre filtre eller [-1] for at afslutte)\n");
		...
		if (inputId == -1)
			...
			printf("Du indtastede ikke et tal. Programmet lukker!\n");
			exit(0);
		}
	} while(inputId != -1);
}
...
void change_filters(filter_typ *filters_arr, struct user_typ user) {
	char answer[MAX_CHAR], user_input_s[MAX_CHAR];
	...
	do{
		...
		, "[10] Lokation"
		, "[11] Hjemmeside");
		scanf(" %s", user_input_s);
		user_input = atoi(user_input_s);
		...
		if (user_input == 1){
			...
			if (user.postnum == 0)		// Checks if the user have given a "postnummer"
				printf("Et postnummer er ikke opgivet, derfor er dette ikke muligt\n\n");
			else {
...
void filter_to_sql(filter_typ *filters_arr, char *filter_str, struct user_typ user) {
	...
	if (strcmp(filters_arr->link, "") != 0 && strcmp(filters_arr->link, "x") != 0){
		if (strcmp(filters_arr->link, "Facebook") == 0)
			strcpy(filters_arr->link, "facebook.dk/marketplace");
...
int go_to_item(sql* sql_group, int len, int inputId, sqlite3 *db, sqlite3_stmt *stmt, char *sql_ori){
	...
	if(select == 1){
		...
		if (buy_item_status == 0)
			printf("FEJL: Kunne ikke koebe produktet\n");
		else {
			printf("Produkt #%d er nu koebt! [id=%d]\n", inputId, sql_group[inputId].id);		// [id=%d] Is only used for testing purposes
...
void initialize_data(sqlite3* db, sqlite3_stmt *stmt, sql db_arr[], char *sql_ori, int len) {
	...
	for (id = 1; id < len; ++id) {
		if (ret == SQLITE_DONE) {
			...
			/* Only testing for len amount of producs */
			if(id >= len)
				break;
		}
...
void print_topX(sql db_arr[], int amount) {
	for (int i = 1; i < amount; ++i)        // Prints the top 20 results
		if (db_arr[i].pris == 0)
			...
			printf("[%d]%s %-15s - %-5s %3dgb - Stand: %-8s - NemID: %3s\n  Pris: %dkr\n"
				, i
				, (i<10) ? " " : ""		// Prints an extra space if "i" is single digit
				, db_arr[i].model
				, db_arr[i].farve
				, db_arr[i].mem
				, db_arr[i].stand
...
struct user_typ login_system(sqlite3 *db) {
	...
	printf("[1] Login    [2] Lav en ny konto    [3] Annonymt login \n-> ");
	while (status == 0) {
		scanf(" %s", scan_answer);
		if (strcmp(scan_answer, "1") == 0) {
		...
		else if (strcmp(scan_answer, "3") == 0) {	// Anonymous login
			user.group = 0;
			user.postnum = 0;
			status = 1;
			printf("\nVelkommen!\n");
			...
			printf("Tast [1] for at logge ind, tast [2] for at lave en ny konto\n");
	}
	return user;
}
struct user_typ ifcreateaccount(sqlite3 *db) {
	...
	char *messageError = "Noget gik i kage!";
	...
	while (user_exists == 0) {
		...
		if (sqlite3_step(selectstmt) == SQLITE_ROW)     // If the password already exists
			printf("Det brugernavn eller password eksisterer allerede. Proev et nyt\n");
		else {                                          // If it doesn't
			user_exists = 1;
			user = give_group();
			// printf("Din gruppe er: %d ----- Postnummer: %d \n", user.group, user.postnum);
			...
			if (exit != SQLITE_OK) {
				printf("FEJL ved oprettelse af konto\n");
				sqlite3_free(messageError);
			}
			else
				printf("Konto oprettet succesfuldt\n\nVelkommen %s\n", username);   /*Get value from database into variable here!*/
		}
	}
...
struct user_typ give_group() {
	...
	printf("Hvad er dit postnummer?  (Hvis du ikke oensker at oplyse dette, skriv derved \"0\"\n-> ");
	scanf(" %d", &postnum);
