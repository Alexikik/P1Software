...
void giveValue(int *value) {
  ...
  printf("%s%s%s%s", "\n\nHvor vigtigt er prisen pae produktet for dig?\n",
          "1. Jeg vurdere andre kriterier som kvalitet eller sikkerhed hoejere end pris\n",
          "2. Prisen har en betydning, men det er ikke afgoerende for om jeg vil koebe produktet\n",
          "3. Prisen paa produktet er det vigtiste kriterie, for om jeg koeber produktet\n");
  scanf("%d", &scan_answer);
  if (scan_answer == 1) {
    *value += 3;
  }
