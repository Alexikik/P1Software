...
void filters(filter_typ *filters_arr, char *filter_str, struct user_typ user);
void select_genbrugs_type(struct user_typ user, char *sql_ori, char filter_str[], filter_typ filters_arr);
...
void genbrugsvare(struct user_typ user, sqlite3 *db) {
	...
	char sql_ori[MAX_CHAR], filter_str[MAX_CHAR];
	filter_typ filters_arr;
	filter_typ *filters_ptr = &filters_arr;
	
	filters(filters_ptr, filter_str, user);
	select_genbrugs_type(user, sql_ori, filter_str, filters_arr);
...
void filters(filter_typ *filters_arr, char *filter_str, struct user_typ user) {
	...
	char sql_temp[MAX_CHAR], answer[MAX_CHAR];
	...
	do{
		...
		if (user_input == 1){
			...
			printf("Hvilken specifikke stand skal produktet vaere i? (Daarlig, Rimelig, God eller Perfekt) \n");
			printf(" (Skriv x for at nulstille filteret)\n-> ");
			scanf(" %[^\n]", answer);
	...
	if (strcmp(filters_arr->min_stand, "") != 0 && strcmp(filters_arr->min_stand, "x") != 0){
		sprintf(sql_temp, " AND Stand='%s'", filters_arr->min_stand);
		strcat(filter_str, sql_temp);
	}
...
// void set_filters_by_group(filter_typ filters_arr, struct user_typ user) {
...
void select_genbrugs_type(struct user_typ user, char *sql_ori, char filter_str[], filter_typ filters_arr){
	if (user.group == 0)		// Group neutral
		sprintf(sql_ori, "%s ORDER BY %s", filter_str, "Pris");
	else if (user.group == 1)	// Group 1
		sprintf(sql_ori, "%s AND Postnummer LIKE '%d%%' ORDER BY %s", filter_str, user.postnum/1000, "Pris"); 	// Done
	else if (user.group == 2)	// Group 2
		sprintf(sql_ori, "%s AND (Stand='God' OR Stand='Perfekt') ORDER BY %s", filter_str, "Pris");	// Done
	else if (user.group == 3)	// Group 3
		sprintf(sql_ori, "%s AND Forsikring='Ja' AND (Nemid='Ja' OR Postnummer LIKE '%d%%') ORDER BY %s", filter_str, user.postnum/1000, "Pris"); 	// Done
	else if (user.group == 12)	// Group 12
		sprintf(sql_ori, "%s AND Postnummer LIKE '%d%%' AND (Stand='God' OR Stand='Perfekt') ORDER BY %s", filter_str, user.postnum/1000, "Pris");
	else if (user.group == 13)	// Group 13
		sprintf(sql_ori, "%s AND Nemid='Ja' AND Forsikring='Ja' AND Postnummer LIKE '%d%%' ORDER BY %s", filter_str, user.postnum/1000, "Pris");
	else if (user.group == 23)	// Group 23
		sprintf(sql_ori, "%s AND Forsikring='Ja' AND (Stand='God' OR Stand='Perfekt') AND (Nemid='Ja' OR Postnummer LIKE '%d%%') ORDER BY %s", filter_str, user.postnum/1000, "Pris");
	else if (user.group == 123)	// Group 123
		sprintf(sql_ori, "%s AND Nemid='Ja' AND Forsikring='Ja' AND Postnummer LIKE '%d%%' AND (Stand='God' OR Stand='Perfekt') ORDER BY %s", filter_str, user.postnum/1000, "Pris");
