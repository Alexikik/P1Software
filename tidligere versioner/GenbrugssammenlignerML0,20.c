...
void select_genbrugs_type(struct user_typ user, char *sql_ori){
	if (user.group == 0)		// Group neutral
		...
		sprintf(sql_ori, "SELECT * from Mobiltelefon WHERE Dato_Solgt='NULL' AND Postnummer LIKE '%d%%' AND (Stand='God' OR Stand='Perfekt') ORDER BY %s", user.postnum/1000, "Pris");
	else if (user.group == 13)	// Group 13
		sprintf(sql_ori, "SELECT * from Mobiltelefon WHERE Dato_Solgt='NULL' AND Nemid='Ja' AND Forsikring='Ja' AND Postnummer LIKE '%d%%' ORDER BY %s", user.postnum/1000, "Pris");
	else if (user.group == 23)	// Group 23
		sprintf(sql_ori, "SELECT * from Mobiltelefon WHERE Dato_Solgt='NULL' AND Forsikring='Ja' AND (Stand='God' OR Stand='Perfekt') AND (Nemid='Ja' OR Postnummer LIKE '%d%%') ORDER BY %s", user.postnum/1000, "Pris");
	else if (user.group == 123)	// Group 123
		sprintf(sql_ori, "SELECT * from Mobiltelefon WHERE Dato_Solgt='NULL' AND Nemid='Ja' AND Forsikring='Ja' AND Postnummer LIKE '%d%%' AND (Stand='God' OR Stand='Perfekt') ORDER BY %s", user.postnum/1000, "Pris");
