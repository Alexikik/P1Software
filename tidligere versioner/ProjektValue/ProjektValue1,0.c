...
int ifcreateaccount(sqlite3 *db) {
	int password_exist = 1, exit, value;
	...
	while (password_exist == 1) {
		...
		sprintf(sql_cmd, "SELECT * FROM User WHERE Password LIKE '%s'", password);
		sqlite3_prepare_v2(db, sql_cmd, -1, &selectstmt, NULL);        // Checks if the password is already used
		// printf("result: %d -- SQLITE_OK: %d -- Step: %d\n", result, SQLITE_OK, sqlite3_step(selectstmt));
